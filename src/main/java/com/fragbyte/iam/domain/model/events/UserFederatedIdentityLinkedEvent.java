...
/**
 * User federated identity linked event.
 *
 * <p>This class represents the event published when a federated identity is linked to a user
 * account.
 *
 ...
 * @since 2026-17-08
 */
public record UserFederatedIdentityLinkedEvent(
    UserId userId,
  ...
  /**
   * The user federated identity linked event constructor.
   *
   * @param userId the identifier of the user.
   * @param provider the external authentication provider.
   * @param providerSubject the unique subject identifier from the provider.
   */
  public UserFederatedIdentityLinkedEvent(
      UserId userId, AuthProvider provider, String providerSubject) {
    this(userId, provider, providerSubject, UUID.randomUUID().toString(), Instant.now());
  }
}
